...
export default {
  ...
  methods: {
    ...
    buyinPriceChange(){
      this.buyinPrice=this.inputNumberFilter(this.buyinPrice, this.currentPricePrecision, 0);
      ...
      if (inputVal && this.state) {
        let cutnum = this.cutNum(balance / inputVal, this.currentVolumePrecision);
    ...
    buyinNumChange(){
      this.buyinNum = this.inputNumberFilter(this.buyinNum, 0, this.currentVolumePrecision);
    ...
    inputNumberFilter (obj, pricePrecision, volumePrecision) {
      let reg
      if (pricePrecision) {
      ...
      this.buyinBtnDisable = true
        ...
        orderCreate(data).then((response) =>{
          if (response.state == 1) {
            ...
            this.$message({
              type: 'success',
              message:this.buyOrSell==='buy'?this.$t('header.BuySuccess'):this.$t('header.SellSuccess'),
              duration: 3000,
              showClose: true
            })
